    ...
    <header className="h-16 bg-card border-b border-border px-6 flex items-center justify-between sticky top-0 z-40">
      ...
      <div className="flex-1 max-w-md mx-4">
        <div className="relative">
          ...
          <input
            ...
            className={`w-full h-10 ${isRTL ? 'pr-11 pl-4' : 'pl-11 pr-4'} rounded-lg bg-muted/50 border border-transparent focus:border-primary/30 focus:bg-card focus:outline-none transition-all text-sm placeholder:text-muted-foreground`}
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        >
          ...
          {notifications > 0 && (
            <span className={`absolute top-1 ${isRTL ? 'right-1' : 'left-1'} w-4 h-4 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center`}>
              {notifications}
            </span>
          )}
        </Link>
        ...
        <button className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted transition-colors">
          ...
          <div className={`hidden md:block ${isRTL ? 'text-right' : 'text-left'}`}>
            ...
            <p className="text-xs text-muted-foreground">{t('common.jobSeeker')}</p>
          </div>

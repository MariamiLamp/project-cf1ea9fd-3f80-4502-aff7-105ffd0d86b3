...
const CompanyDashboard = () => {
  ...
  const filteredApplications = applications.filter((app) => {
    if (appliedFilters.status !== "all" && app.status !== appliedFilters.status) return false;
    if (appliedFilters.job !== "all" && app.jobTitle !== appliedFilters.job) return false;
    if (appliedFilters.date && app.appliedDate !== appliedFilters.date) return false;
    if (appliedFilters.matchScore !== "all" && app.matchScore < parseInt(appliedFilters.matchScore)) return false;
    return true;
  });
  ...
  const handleAddJobData = (data: JobFormData) => {
    if (!data.title || !data.department || !data.type || !data.country || !data.city) {
    ...
    setNewJob({ title: "", department: "", type: "", country: "", city: "", description: "", requirements: "" });
  };
  ...
  const handleEditJob = (job: any) => {
    setSelectedJob(job);
    const [country, city] = job.location.includes(" - ") ? job.location.split(" - ") : [job.location, ""];
    setEditJob({
      id: job.id,
      title: job.title,
  ...
  const handleSaveEditData = (data: JobFormData) => {
    ...
    toast({ title: "تم التحديث", description: "تم تحديث بيانات الوظيفة بنجاح" });
  ...
  const handleToggleJobStatus = (jobId: number) => {
    ...
    toast({ title: "تم تحديث الحالة", description: "تم تحديث حالة الوظيفة بنجاح" });
  };
  ...
  const handleUpdateApplicationStatus = (
    ...
    setApplications(applications.map((app) => (app.id === appId ? { ...app, status, ...additionalData } : app)));
  ...
  const handleGenerateDescription = async () => {
    if (!jobGenTitle) {
      toast({ title: "خطأ", description: "يرجى إدخال المسمى الوظيفي", variant: "destructive" });
      return;
    }
...
• فرص للتطوير المهني`;
  ...
  const handleAddAd = () => {
    if (!adForm.imageUrl) {
      toast({ title: "خطأ", description: "يرجى إدخال رابط صورة البانر", variant: "destructive" });
      return;
    }
    ...
    setAdForm({ title: "", description: "", imageUrl: "", link: "", placement: "hero-bottom", duration: "1_month" });
  };
                ...
                <div className="space-y-2 text-right">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (!newJob.title) {
                          toast({ title: "خطأ", description: "يرجى إدخال المسمى الوظيفي أولاً", variant: "destructive" });
                          return;
                        }
                        setIsGenerating(true);
                        setTimeout(() => {
                ...
                <PopoverTrigger asChild>
                  <Button variant="outline" size="sm" className="h-8 gap-2 bg-background">
                    ...
                    {(appliedFilters.status !== "all" ||
                      ...
                      <Badge variant="secondary" className="mr-1 h-5 px-1 text-[10px] rounded-sm pointer-events-none">
                        مفعل
                      </Badge>
                    )}
                  </Button>
                ...
                <PopoverContent className="w-80 p-4" align="end">
                  <div className="space-y-4" dir="rtl">
                    <div className="space-y-2 border-b pb-2">
                      <h4 className="font-medium leading-none">خيارات التصفية</h4>
                      <p className="text-sm text-muted-foreground">قم بتصفية طلبات التوظيف حسب رغبتك</p>
                    </div>
                    <div className="grid gap-3">
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="job" className="text-xs">الوظيفة</Label>
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="date" className="text-xs">التاريخ</Label>
                        <Input id="date" type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="h-8" />
                      </div>
                      <div className="space-y-1.5">
                        <Label htmlFor="match" className="text-xs">نسبة التوافق</Label>
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" className="flex-1 gap-2" onClick={handleApplyFilters}>
                        <ListFilter className="w-4 h-4" />
                        تصفية
                      </Button>
                      {(appliedFilters.status !== "all" ||
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setAppliedFilters({ status: "all", job: "all", date: "", matchScore: "all" });
                          }}
                        >
            ...
            <div className="hidden md:flex flex-wrap items-center justify-start gap-3 w-full">
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-2 border-r pr-2 mr-2">
                  ...
                  <Button size="sm" onClick={handleApplyFilters} className="h-8 w-[130px] gap-2">
                ...
                <Input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="w-[150px] h-8 text-xs" />
            ...
            {filteredApplications.length > 0 ? (
              filteredApplications.map((app) => (
                <TableRow key={app.id}>
                  <TableCell>
                    <div className="flex gap-1 justify-end">
                      <Button variant="ghost" size="sm" onClick={() => handleViewApplication(app)} className="gap-2 h-8 px-3">
                  ...
                  <TableCell>
                    <Badge variant={app.matchScore >= 90 ? "default" : "secondary"}>{app.matchScore}%</Badge>
                  </TableCell>
                  <TableCell>{app.jobTitle}</TableCell>
                  <TableCell className="font-medium">
                    <button onClick={() => handleViewCandidate(app.candidateId)} className="text-primary hover:underline cursor-pointer font-medium">
                      {app.candidateName}
                    </button>
                  </TableCell>
                </TableRow>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="text-right">
          ...
          <CardDescription className="text-right">قم بإنشاء بانر ترويجي لشركتك يظهر في الصفحة الرئيسية</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div className="h-[600px] sticky top-4">
              <AdPlacementSelector selectedPlacement={adForm.placement} onSelect={(value) => setAdForm({ ...adForm, placement: value })} adImage={adForm.imageUrl} />
            </div>

            <div className="space-y-4">
              <div className="space-y-2 text-right">
                <Label>عنوان البانر</Label>
                <Input className="text-right" placeholder="مثال: انضم لفريقنا المميز" value={adForm.title} onChange={(e) => setAdForm({ ...adForm, title: e.target.value })} />
              </div>
              ...
              <div className="space-y-2 text-right">
                <Label>المدة</Label>
                <Select value={adForm.duration} onValueChange={(value) => setAdForm({ ...adForm, duration: value })}>
                ...
                <p className="text-xs text-muted-foreground mt-1">مدة عرض الإعلان بالأشهر</p>
              </div>

              <div className="space-y-2 text-right">
                <Label>رابط الوجهة عند النقر (اختياري)</Label>
                <Input className="text-right" dir="ltr" placeholder="https://" value={adForm.link} onChange={(e) => setAdForm({ ...adForm, link: e.target.value })} />
              </div>
                    ...
                    <TableRow key={job.id}>
                      <TableCell>
                        <div className="flex justify-end">
                          <Button variant={isFeatured ? "outline" : "default"} size="sm" onClick={() => handleToggleFeatured(job.id)} className="gap-1">
  ...
  const renderAds = () => (
    ...
    <Card>
      <CardHeader className="text-right">
        ...
        <CardDescription className="text-right">قائمة الإعلانات المنشورة للموقع</CardDescription>
      </CardHeader>
      <CardContent>
        {liveAds.length === 0 ? (
          <p className="text-muted-foreground text-right">لا توجد إعلانات حالياً</p>
        ) : (
          <div className="grid gap-2">
            {liveAds.map((a) => (
              <div key={a.id} className="flex items-center justify-between p-2 border rounded-md">
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" onClick={() => handleRemoveAd(a.id)} className="text-destructive">
                    حذف
                  </Button>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-24 h-12 rounded overflow-hidden bg-muted">
                    <img src={a.imageUrl} alt={a.title || "ad"} className="w-full h-full object-cover" />
                  </div>
                  <div className="text-right">
                    <div className="font-medium">{a.title || "بدون عنوان"}</div>
                    <div className="text-xs text-muted-foreground">{a.placement === "hero-bottom" ? "تحت الهيرو" : "تحت فحص الـATS"}</div>
                  </div>
                </div>
              </div>
            ))}
  ...
  const renderGenerator = () => (
    <Card>
      <CardHeader className="text-right">
        ...
        <CardDescription className="text-right">استخدم الذكاء الاصطناعي لتوليد وصف وظيفي احترافي</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid md:grid-cols-3 gap-4">
          <div className="space-y-2 text-right">
            <Label>المسمى الوظيفي *</Label>
            <Input className="text-right" placeholder="مثال: مطور واجهات أمامية" value={jobGenTitle} onChange={(e) => setJobGenTitle(e.target.value)} />
          </div>
          <div className="space-y-2 text-right">
            <Label>القسم</Label>
            <Input className="text-right" placeholder="مثال: التقنية" value={jobGenDepartment} onChange={(e) => setJobGenDepartment(e.target.value)} />
          </div>
          <div className="space-y-2 text-right">
            <Label>سنوات الخبرة</Label>
            <Input className="text-right" placeholder="مثال: 3+" value={jobGenExperience} onChange={(e) => setJobGenExperience(e.target.value)} />
          </div>
        </div>

        <Button onClick={handleGenerateDescription} disabled={isGenerating} className="gap-2">
          {isGenerating ? (
            <>
        ...
        {generatedDescription && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Button variant="outline" size="sm" onClick={handleCopyDescription} className="gap-1">
                نسخ
              </Button>
              <Label>الوصف المُولَّد</Label>
            </div>
            <Textarea value={generatedDescription} readOnly className="text-right min-h-[300px]" />
          </div>
        )}
      </CardContent>
    </Card>
      ...
      <div className="space-y-6">
        ...
        <Dialog open={isEditJobOpen} onOpenChange={setIsEditJobOpen}>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto" dir="rtl">
            <DialogHeader className="text-right sm:text-right">
              ...
              <DialogDescription className="text-right">قم بتحديث بيانات الوظيفة</DialogDescription>
            </DialogHeader>
            <JobForm mode="edit" initialData={editJob} onSubmit={handleSaveEditData} submitLabel="حفظ التغييرات" />
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isViewApplicationOpen} onOpenChange={setIsViewApplicationOpen}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto" dir="rtl">
            <DialogHeader>
              <DialogTitle className="text-right">تفاصيل طلب التوظيف</DialogTitle>
            </DialogHeader>
            {selectedApplication && (
              <Tabs defaultValue="details" className="w-full mt-4">
                ...
                <TabsContent value="details" className="space-y-4 text-right">
                  ...
                  <Button variant="outline" className="w-full gap-2" onClick={() => handleViewCandidate(selectedApplication.candidateId)}>
                  ...
                  {selectedApplication.status === "rejected" && selectedApplication.rejectionReason && (
                    <div className="p-4 bg-red-50 border border-red-100 rounded-lg space-y-2">
                      ...
                      <p className="text-sm text-red-600 italic">"{selectedApplication.rejectionReason}"</p>
                    </div>
                  )}

                  {(selectedApplication.status === "shortlisted" || pendingAction === "shortlisted") && (
                    <div className="p-4 bg-amber-50 border border-amber-100 rounded-lg space-y-3">
                      <Label className="text-amber-800 font-bold">ملاحظات الشركة عن المرشح:</Label>
                      <Textarea placeholder="أضف ملاحظاتك هنا..." className="bg-white border-amber-200 focus:ring-amber-500" value={notesInput} onChange={(e) => setNotesInput(e.target.value)} />
                    </div>
                  )}
                  ...
                  {(selectedApplication.status === "pending" ||
                    ...
                    <div className="flex flex-col gap-3 pt-4 border-t">
                      <div className="space-y-2">
                        <Label className="text-xs text-muted-foreground mr-1">سبب الرفض (اختياري - يظهر فقط في حال الرفض):</Label>
                        <Input placeholder="مثال: الخبرة غير كافية..." className="h-8 text-xs bg-muted/20" value={rejectionReasonInput} onChange={(e) => setRejectionReasonInput(e.target.value)} />
                      </div>

                      <div className="space-y-2">
                        <Label className="text-sm font-medium">إجراءات الطلب:</Label>
                        <Select value={pendingAction || ""} onValueChange={(value) => setPendingAction(value)}>
                          ...
                          <SelectContent dir="rtl">
                            <SelectItem value="accepted" className="text-emerald-600 focus:text-emerald-700 focus:bg-emerald-50 cursor-pointer">
                              <div className="flex items-center gap-2 py-1">
                                <CheckCircle className="w-4 h-4" />
                                <span className="font-medium">قبول الطلب</span>
                              </div>
                            </SelectItem>
                            <SelectItem value="rejected" className="text-red-600 focus:text-red-700 focus:bg-red-50 cursor-pointer">
                              <div className="flex items-center gap-2 py-1">
                                <XCircle className="w-4 h-4" />
                                <span className="font-medium">رفض الطلب</span>
                              </div>
                            </SelectItem>
                            <SelectItem value="shortlisted" className="text-amber-600 focus:text-amber-700 focus:bg-amber-50 cursor-pointer">
                              <div className="flex items-center gap-2 py-1">
                                <Star className="w-4 h-4" />
                                <span className="font-medium">إضافة للقائمة المختصرة</span>
                              </div>
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (pendingAction) {
                            const additionalData = pendingAction === "rejected" ? { rejectionReason: rejectionReasonInput } : {};
                            handleUpdateApplicationStatus(selectedApplication.id, pendingAction, additionalData);
                            if (pendingAction === "accepted" || pendingAction === "rejected") {
                              setIsViewApplicationOpen(false);
                            }
                            setPendingAction(null);
                          } else {
                            handleUpdateApplicationStatus(selectedApplication.id, selectedApplication.status, { rejectionReason: rejectionReasonInput, notes: notesInput });
                          }
                        }}
                      >
                  ...
                  <div className="bg-muted/30 border rounded-xl p-4 min-h-[500px] flex flex-col">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">CV_{selectedApplication.candidateName.replace(" ", "_")}.pdf</span>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 bg-background rounded-lg border shadow-sm p-8 space-y-6">
                      ...
                      <div className="mt-8 pt-8 text-center border-t border-dashed">
                        <p className="text-xs text-muted-foreground uppercase tracking-widest">Candidate Resume Preview</p>
                      </div>
                    </div>
                  </div>
                </TabsContent>
                  ...
                  <div className="bg-muted/30 border rounded-xl p-4 min-h-[500px] flex flex-col">
                    <div className="bg-background rounded-lg border shadow-sm p-10 flex-1 flex flex-col">
                      <div className="mb-8 border-b pb-6">
                        <h4 className="text-xl font-bold text-primary mb-1 text-right">خطاب التقديم</h4>
                        <p className="text-sm text-muted-foreground text-right">تاريخ التقديم: {selectedApplication.appliedDate}</p>
                      </div>
                      ...
                      <div className="flex-1 text-right">
                        <p className="text-sm leading-relaxed whitespace-pre-line text-foreground/80 font-serif text-right">
                          ...
                          أتقدم إليكم بطلب للانضمام إلى فريقكم الموقر في وظيفة {selectedApplication.jobTitle}.{"\n\n"}
                          لدي شغف كبير وخبرة عملية في هذا المجال، وأتطلع بحماس للمساهمة في تحقيق أهداف شركتكم. لقد تابعت إنجازات شركتكم بإعجاب، وأرى أن مهاراتي تتناسب تماماً مع متطلبات الوظيفة المعلنة.{"\n\n"}
                          خلال مسيرتي المهنية، قمت بالعديد من المشاريع الناجحة التي أثبتت قدرتي على العمل الجاد والالتزام بأعلى معايير الجودة.{"\n\n"}
                          أتطلع لمناقشة كيف يمكنني أن أكون إضافة قيمة لفريقكم.{"\n\n"}
                          مع خالص التحية والتقدير،{"\n\n"}
                          <span className="font-bold text-lg text-primary block text-right">{selectedApplication.candidateName}</span>
                        </p>
                      </div>
